...
**Resolved** — see "Classification alignment" in the [modeling log](./MODELING-LOG.md).
A full-corpus census confirms equal counts in 100% of
...
the seven disagreement kinds — `RequiredFieldNotLocated`,
`NonRepeatingFieldRepeated`, `InvalidAccession`, `InvalidFieldValue`,
`RepetitionLimitExceeded`, `RelatedFieldsDisagree`, and `RpaCodeNotAttested`
(the last for records checked against a fiscal-year RPA vocabulary) — each carry a
source-linked `RuleRef`; alongside them `RuleUnresolved`, `FieldNotYetModeled`, and
`TagNotAscii` carry the examined `Location` instead. `src/report.gleam` renders
each record's Identity, Participants, Chronology, Classifications, and Narratives,
plus the top-level Project result (or their problems), a summary, and — when the
corpus fiscal year is inferred from the file name — an attestation-mode note. Narratives are
...
How this reading was built and stress-tested — the rule-inventory batches, the
checked constructor's runs against the FY88/FY89/FY94 corpora, the held-out FY94
and FY89 generalization tests and their reconciliations, the classification-
alignment census, and the maintainability review — is recorded in
[MODELING-LOG.md](./MODELING-LOG.md). That is the dated derivation trail; this
README states the current design.
