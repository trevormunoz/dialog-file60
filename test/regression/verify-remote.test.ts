...
// Every phrase index stub above has zero terms, so a matching phrase-counts.json records zero
// for every code too -- the same "manifest silent" shape checkPhraseManifest's own doc comment
...
const okIndexes = Object.fromEntries([
  ...
  ["https://corpus.example/v1/phrase-counts.json", JSON.stringify({ phraseTerms: matchingPhraseTerms })],
...
// hard-fails when a loaded phrase index disagrees with phrase-counts.json's phraseTerms, so
// this catches that disagreement before it ships instead of at first boot.
test("a live phrase-counts.json missing phraseTerms fails verify", async () => {
  const missingPhraseTerms = { ...okIndexes };
  missingPhraseTerms["https://corpus.example/v1/phrase-counts.json"] = '{"file":"RG164.CRIS.FY94.txt"}';
  await expect(verifyRemote("https://corpus.example/v1/", {
    fetchImpl: stubFetch(synthetic, missingPhraseTerms),
    expected: syntheticFixity,
    fixture: FIXTURE,
    fixtureOffset: FIXTURE_OFFSET,
  })).rejects.toThrow(/phrase-counts\.json: missing phraseTerms/);
});

test("a phrase index whose term count disagrees with phrase-counts.json's phraseTerms fails verify", async () => {
  ...
  await expect(verifyRemote("https://corpus.example/v1/", {
    ...
    fixture: FIXTURE,
    fixtureOffset: FIXTURE_OFFSET,
  })).rejects.toThrow(new RegExp(`index/${code}\\.json: 1 terms loaded, phrase-counts\\.json phraseTerms expected 0`));
});

test("matching phrase-counts.json phraseTerms and index term counts pass verify", async () => {
  await expect(verifyRemote("https://corpus.example/v1/", {
    fetchImpl: stubFetch(synthetic, okIndexes),
    expected: syntheticFixity,
    fixture: FIXTURE,
