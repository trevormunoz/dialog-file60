...
registry.get("proto.accounting.prints");
...
export async function runLogoff(session: DialogSession): Promise<OutputLine[]> {
  ...
  const keys = ["proto.logoff.template", "rates.file60_1998", "proto.accounting.combination", "proto.accounting.prints", "proto.session.user_number", "proto.session.clock"];
  const out: OutputLine[] = session.accounting
    ? logoffBlock({ start: session.start, end, user: session.user, types: session.typeCounts, prints: session.printCounts, rates: registry.get("rates.file60_1998").value as Rates })
