...
export class FetchRangeReader implements RangeReader {
  ...
  async read(offset: number, length: number): Promise<Uint8Array> {
    ...
    if (res.status !== 206) throw new Error(`range request not honored: HTTP ${res.status}`);
    const bytes = new Uint8Array(await res.arrayBuffer());
    if (bytes.length !== length) {
      throw new Error(`short range response: ${bytes.length} of ${length} bytes requested at offset ${offset} from ${this.url}`);
    }
    return bytes;
  }
}
